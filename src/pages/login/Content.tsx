...
const ConfirmOtpModal = React.lazy(() => import('./modal/ConfirmOtpModal'));
// const ForceChangePasswordModal = React.lazy(() => import('./modal/ForceChangePasswordModal'));
const AccountModal = React.lazy(() => import('./modal/AccountModal'));
const USLSPrivacyModal = React.lazy(() => import('./modal/USLSPrivacyModal'));
const SignInWithGoogle = React.lazy(() => import('./signInGoogle'));
const SignUpModal = React.lazy(() => import('./signUpWithModal'));
  ...
  const login = async (
    ...
    try {
      ...
      if (responseData.selected_user) {
        setSavedUser(responseData.selected_user);
      ...
      if (responseData.is_otp) {
      ...
      } else if (responseData.is_multiple_accounts) {
        setAccountModal(true);
        setAccounts(responseData.user || []);
        // } else if (responseData.first_login) {
        //   setOpenForceChangePasswordModal(true);
      } else if (responseData.show_privacy_notice) {
        if (accountModal) {
          setAccountModal(false);
        }
        ...
        if (
          ...
          responseData.includes('Account Not Yet Verified. Check your email to verify you account.')
        ) {
          if (accountModal) {
            setAccountModal(false);
          }
      ...
      <div className='shadow-lg rounded-[30px] md:mt-0 xl:w-[500px] lg:w-[500px] sm:w-[400px] px-8 py-5 z-10 bg-white'>
        ...
        <div className='px-8 pb-6 space-y-2'>
          ...
          <div>
            <form onSubmit={login}>
              ...
              <div className='flex justify-between'>
                <h1
                  id='forgot-password'
                  className='text-sm font-semibold text-blue-600 hover:underline hover:cursor-pointer float-right mb-5'
                  tabIndex={4}
          ...
          <div className='flex flex-row space-x-4 justify-center pt-4'>
            <img src={static_images.eu_gdpr_logo} alt='GDPR' width={100} height={100} />
            <img src={static_images.iso_27001} alt='ISO' width={100} height={100} />
          </div>
        </div>
      </div>
      {openSignUpModal && (
        <Suspense fallback={<></>}>
          <SignUpModal isOpen={openSignUpModal} onClose={() => setOpenSignUpModal(false)} />
        </Suspense>
      )}
      {openOtpModal && (
        <Suspense fallback={<></>}>
          <ConfirmOtpModal
            ...
            resend={() => login(null, savedUser, true)}
            submit={(otp: string) => login(null, savedUser, false, false, otp)}
          />
        </Suspense>
      )}
      ...
      {showUSLSPrivacyModal && (
        <Suspense fallback={<></>}>
          <USLSPrivacyModal
            ...
            handleLogin={() => login(null, savedUser, false, true)}
          />
        </Suspense>
      )}
    </div>
