...
interface ModalProps {
  response: any;
  isOpen: boolean;
  handleLogin: (event: React.FormEvent<HTMLFormElement> | null, selectedUser: any) => Promise<void>;
  onClose: () => void;
}

const AccountModal: FC<ModalProps> = ({ response, handleLogin, isOpen, onClose }: any) => {
  const cancelButtonRef = useRef(null);

  const renderAccountsList = () => {
    return response.map((account: any) => (
      <div className='flex justify-between border-b-2 border-[#C7C7C7] p-2 mb-3 cursor-pointer text-black hover:text-blue-600' onClick={() => handleLogin(null, account)} key={account.id}>
