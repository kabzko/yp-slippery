...
const Division = () => {
  ...
  type SortConfig = {
    key: keyof (typeof divisionData.divisions)[0] | null;
    direction: 'asc' | 'desc';
  };

  const [sortConfig, setSortConfig] = React.useState<SortConfig>({ key: null, direction: 'asc' });

  const handleSort = (key: keyof (typeof divisionData.divisions)[0]) => {
  ...
  const sortedData = React.useMemo(() => {
    ...
    return [...divisionData.divisions].sort((a, b) => {
  ...
  const handleSelectedAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.checked) {
      const idsToSelect = sortedData?.length > 0 ? sortedData.map((division: any) => division.id) : [];
  ...
  const handleSelected = (e: React.ChangeEvent<HTMLInputElement>, division: any) => {
    if (division.id) {
      if (e.target.checked) {
        setSelectedRows((prev: any[]): any[] => [...prev, division.id]);
      } else {
        setSelectedRows((prev: any[]): any[] => prev.filter((id) => id !== division.id));
      }
    }
  };
  ...
  const handleDownload = async () => {
    window.location.href =
      'https://yp-files.s3.ap-southeast-1.amazonaws.com/yp-slippery-import-templates/Setup+-+Division.csv';
    toast.custom(() => <CustomToast message={`Successfully downloaded division csv.`} type='success' />, {
      duration: 4000,
    });
  };
        ...
        <div className='flex flex-col mx-14 lg:grid mb-[0.5rem]'>
          ...
          <div className='flex flex-row justify-between my-5'>
            <div className='flex items-center'>
              <h1>Total Record/s: {divisionData?.pagination.total_records}</h1>
            </div>
            <div className='flex gap-2 items-center'>
              <Page
                currentPage={currentPage}
                totalPages={divisionData?.pagination.total_pages}
