...
const SubUnits = () => {
  ...
  type SortConfig = {
    key: keyof (typeof subUnitsData.subUnits)[0] | null;
    direction: 'asc' | 'desc';
  };

  const [sortConfig, setSortConfig] = React.useState<SortConfig>({ key: null, direction: 'asc' });

  const handleSort = (key: keyof (typeof subUnitsData.subUnits)[0]) => {
  ...
  const sortedData = React.useMemo(() => {
    if (!subUnitsData || !subUnitsData.subUnits) return [];

    return [...subUnitsData.subUnits].sort((a, b) => {
  ...
  const handleSelectedAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.checked) {
      ...
      setSelectedRows(dataToUse.map((subUnit: any) => subUnit.id));
    } else {
      setSelectedRows([]);
    }
  };

  const handleSelected = (e: React.ChangeEvent<HTMLInputElement>, subUnit: any) => {
    if (subUnit.id) {
      if (e.target.checked) {
        setSelectedRows((prev: any[]): any[] => [...prev, subUnit.id]);
      } else {
        setSelectedRows((prev: any[]): any[] => prev.filter((id) => id !== subUnit.id));
      }
    }
  };
  ...
  const handleDownload = async () => {
    window.location.href =
      'https://yp-files.s3.ap-southeast-1.amazonaws.com/yp-slippery-import-templates/Setup+-+SubUnit.csv';
    toast.custom(() => <CustomToast message={`Successfully downloaded subunit csv.`} type='success' />, {
      duration: 4000,
    });
  };
        ...
        <div className='flex flex-col mx-14 lg:grid mb-[0.5rem]'>
          ...
          <div className='flex flex-row justify-between my-5'>
            <div className='flex items-center'>
              <h1>Total Record/s: {subUnitsData?.pagination.total_records}</h1>
            </div>
            <div className='flex gap-2 items-center'>
              <Page
                currentPage={currentPage}
                totalPages={subUnitsData?.pagination.total_pages}
