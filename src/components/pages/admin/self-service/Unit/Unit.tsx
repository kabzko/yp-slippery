...
const Unit = () => {
  ...
  const { data: unitsData, refetch } = useGetUnitsData(currentPage, pageSize);
  ...
  const handleDownload = async () => {
    const endpoint = 'https://yp3.theabbaaccounting.com/api/units/export-csv/';
    try {
      const response = await axios.get(endpoint, { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'setup-unit.csv');
  ...
  return (
    <>
      <Upload fields={["Units"]} isOpen={openUploadModal} onClose={closeUpload} />
        ...
        <div className="mx-14 border-b-2 xl:flex border-stone-700 lg:max-w-full xl:justify-between lg:grid-col">
          <div className="flex">
            ...
            <div className="flex relative my-7 ml-5 group">
              ...
              <span className="absolute scale-0 w-[400px] rounded-lg drop-shadow-lg border border-[#ACB9CB] bg-slate-400 p-2 pl-4 text-sm text-black font-bold group-hover:scale-100" style={{ bottom: '40px', left: '-200px' }}>List every unit for your business.</span>
            </div>
          </div>
        ...
        <div className="flex flex-col mx-14 lg:grid">
          <div className="overflow-auto max-h-64 bg-white rounded">
            <table className="w-full border-b-2 table-auto">
                ...
                <tr>
                  <th scope="col" className="px-3 py-3.5">
                    <input 
                      ...
                      checked={selectedRows?.length === (sortedData?.length || 0)}
                    />
                  </th>
                  <th scope="col" className="px-3 py-3.5">
              ...
              <tbody>
                {sortedData?.length > 0 ? sortedData.map((unit: any) => (
                  <tr key={unit.id} className="p-4 text-center border-b border-blue-gray-50">
                    ...
                    <td className="py-1">
                      <div className="flex justify-center space-x-2">
                        <button 
                          onClick={() => { setSelectedUnit([unit]); setSelectedRows([unit.id]); openEditModal(); }} 
                          id="editbtn" 
                        ...
                        <button 
                          onClick={() => { setSelectedUnit([unit]); setSelectedRows([unit.id]); openDeleteModal(); }} 
                          id="deletebtn" 
          ...
          <div className="flex flex-row justify-between mx-5 my-5">
            <div className="flex items-center">
              <h1>
                Total Record/s: {unitsData?.units?.length || 0}
              </h1>
            </div>
            ...
            <div className="flex gap-2 items-center">
              ...
              <button 
                onClick={() => setCurrentPage(prev => Math.min(prev + 1, unitsData?.pagination.total_pages || 1))}
                disabled={currentPage === (unitsData?.pagination.total_pages || 1)}
                className="p-1 rounded-full hover:bg-gray-100 disabled:opacity-50"
              >
