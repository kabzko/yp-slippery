...
const Schedule = () => {
  ...
  const pageSize = 10;
  const { data: schedulesDatas, isLoading } = useGetScheduleData(currentPage, pageSize);
    ...
    return [...schedulesDatas.schedule].sort((a, b) => {
      ...
      if (sortConfig.key === 'timein' || sortConfig.key === 'timeout') {
        return sortConfig.direction === 'asc'
          ...
          : new Date(`1970-01-01T${bValue}`).getTime() - new Date(`1970-01-01T${aValue}`).getTime();
        ...
        return sortConfig.direction === 'asc' ? (aWorkdays < bWorkdays ? -1 : 1) : aWorkdays > bWorkdays ? -1 : 1;
      } else {
  ...
  const handleDeleteSelected = () => {
    ...
    const temp = selectedRows
      ?.map((id: any) => schedulesDatas.schedule.find((item: any) => item.id === id))
      .filter(Boolean);
  ...
  const handleSelected = (e: React.ChangeEvent<HTMLInputElement>, schedule: any) => {
    if (schedule.id) {
      if (e.target.checked) {
        setSelectedRows((prev: any[]): any[] => [...prev, schedule.id]);
      } else {
        setSelectedRows((prev: any[]): any[] => prev.filter((id) => id !== schedule.id));
      }
    }
  };
  ...
  const handleDownload = async () => {
    window.location.href =
      'https://yp-files.s3.ap-southeast-1.amazonaws.com/yp-slippery-import-templates/Setup+-+Schedule.csv';
    toast.custom(() => <CustomToast message={`Successfully downloaded setup schedule csv.`} type='success' />, {
      duration: 4000,
    });
  };
  ...
  return (
    <>
      <Upload fields={schedFields} isOpen={openUploadModal} onClose={closeUpload} module='schedule' />
      <CreateScheduleModal isOpen={isModalOpen} onClose={closeModal} />
      <EditScheduleModal data={selectedSched} isOpen={editModal} onClose={closeEditModal} />
      <DeleteModal selectedSched={selectedSched} isOpen={deleteModal} onClose={closeDeleteModal} />
        ...
        <div className='flex flex-col mx-14 lg:grid mb-[0.5rem]'>
              ...
              <tbody>
                {isLoading ? (
                  ...
                  sortedData?.map((schedule: any) => (
                    <tr key={schedule.id} className='p-4 text-center border-b border-blue-gray-50'>
                      <td>
                        <input
                          type='checkbox'
                      ...
                      <td className='py-1'>{schedule.flexible_time ? 'Yes' : 'No'}</td>
                      <td className='py-1'>{schedule.flexible_breaktime || schedule.fixed_breaktime ? 'Yes' : 'No'}</td>
                      <td className='py-1'>
          ...
          <div className='flex flex-row justify-between my-5'>
            <div className='flex items-center'>
              <h1>Total Record/s: {schedulesDatas?.pagination.total_records}</h1>
            </div>
            <div className='flex gap-2 items-center'>
              <Page
                currentPage={currentPage}
                totalPages={schedulesDatas?.pagination.total_pages}
