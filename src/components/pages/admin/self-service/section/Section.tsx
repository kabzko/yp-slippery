...
const Section = () => {
  ...
  const handleSelectedAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.checked) {
      const idsToSelect = sortedData?.length > 0 ? sortedData.map((section: any) => section.id) : [];
  ...
  const handleDownload = async () => {
    window.location.href =
      'https://yp-files.s3.ap-southeast-1.amazonaws.com/yp-slippery-import-templates/Setup+-+Section.csv';
        ...
        <div className='flex flex-col mx-14 lg:grid mb-[0.5rem]'>
          ...
          <div className='flex flex-row justify-between my-5'>
            <div className='flex items-center'>
              <h1>Total Record/s: {sectionsData?.pagination.total_records}</h1>
            </div>
            <div className='flex gap-2 items-center'>
              <Page
                currentPage={currentPage}
                totalPages={sectionsData?.pagination.total_pages}
