...
export default function DeleteModal({ section, isOpen, onClose }: ModalProps) {
  ...
  const { refetch } = useGetSectionData(1, 10);
  const queryClient = useQueryClient();

  useEffect(() => {
    if (section && selectedRows.length === 1) setSectionName(section[0].name)
              ...
              <div className="inline-block overflow-hidden text-left align-bottom bg-white rounded-lg shadow-xl transition-all transform sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:pb-6">
                <div className="pb-5 text-center sm:text-left">
                  ...
                  <h1 className="px-20 text-xl text-center">
                    {selectedRows.length > 1 ? "Are you sure you want to delete these items?":`Are you sure you want to delete ${sectionName}?`}
                  </h1>
                </div>
